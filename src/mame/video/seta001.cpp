...
void seta001_device::draw_background( bitmap_ind16 &bitmap, const rectangle &cliprect, int bank_size)
{
	...
	std::ofstream fout("/home/jtejada/git/jtcores/cores/kiwi/ver/game/seta_cfg.hex");
	for (int k=0; k<4; k++) fout << std::hex << (unsigned)m_spritectrl[k] << '\n';
	fout.close();
	fout.open("/home/jtejada/git/jtcores/cores/kiwi/ver/game/vram_lo.bin");
	fout.write( (const char*) m_spritecodelow.get(), 0x1000 );
	fout.close();
	fout.open("/home/jtejada/git/jtcores/cores/kiwi/ver/game/vram_hi.bin");
	fout.write( (const char*) m_spritecodehigh.get(), 0x1000 );
	fout.close();
	fout.open("/home/jtejada/git/jtcores/cores/kiwi/ver/game/col.bin");
